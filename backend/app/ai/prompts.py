...
def build_system_prompt(settings: StoreSettings, products: List[Product]) -> str:
    ...
    base_instructions = settings.system_prompt if settings.system_prompt else (
        "შენ ხარ პროფესიონალი AI გაყიდვების ასისტენტი ვეიპის სითხეების (E-Liquids) მაღაზიაში. "
        "შენი მიზანია მომხმარებელს დაეხმარო სწორი არომატისა და ნიკოტინის დონის შერჩევაში, "
        "გააცნო პროდუქცია და მიიყვანო შეკვეთამდე."
    )
